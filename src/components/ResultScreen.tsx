...
function MedicalRiskSection({
  medicalRisk,
}: {
  medicalRisk: SimulationResult['medicalRisk'];
  tier?: ProTier;
}) {
  const {
    monthlyCostAfter80,
    dementiaCost,
    monthlyProtectionInsurance,
    uncoveredRisk,
    coverageScore,
  } = medicalRisk;

  const coverageColor = coverageScore >= 70
    ? 'text-emerald-600'
    : coverageScore >= 40
      ? 'text-amber-600'
      : 'text-red-600';

  const coverageBg = coverageScore >= 70
  ...
  return (
    <section className="rounded-2xl border border-navy-100 bg-white p-4 space-y-4">
      ...
      <div className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2.5">
        ...
        <p className="text-[15px] font-extrabold text-rose-600">{formatKRW(monthlyCostAfter80)}</p>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2.5">
        ...
        <p className="text-[15px] font-extrabold text-rose-600">{formatKRW(dementiaCost)}</p>
      </div>
      ...
      <div className={`rounded-xl border p-3 ${coverageBg}`}>
        ...
        <div className="h-2.5 rounded-full bg-white/80 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-700 ${
              coverageScore >= 70 ? 'bg-emerald-500' : coverageScore >= 40 ? 'bg-amber-500' : 'bg-red-500'
            }`}
        ...
        <div className="mt-2 flex justify-between text-[10px]">
          ...
          <span className={`font-bold ${coverageColor}`}>
            {coverageScore >= 70 ? '양호' : coverageScore >= 40 ? '보완 필요' : '위험'}
          </span>
        </div>
      </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-red-500 mt-1">
                현재 보장 수준으로는 3대 질병 발생 시 이 금액이 본인 부담이 될 수 있습니다.
                전문가 상담을 통해 보장 구조를 점검해 보세요.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-navy-100 bg-navy-50 px-3 py-2.5">
        <p className="text-[11px] font-bold text-navy-700 leading-relaxed">
          한국 노인 자살률 OECD 1위 — 주요 원인 1위 생활비, 2위 치료비
        </p>
        <p className="text-[10px] text-navy-500 mt-1 leading-relaxed">
          은퇴설계는 현금흐름과 의료비 보장, 두 가지를 함께 준비해야 완성됩니다.
        </p>
      </div>
    </section>
  );
...
export default function ResultScreen({ result: initialResult, onBack, tier = 'plus' }: Props) {
  ...
  const graphData = yearRows.map((r) => ({
    ...
    전액과세: r.isAccumulationPhase ? Math.round(r.balance) : Math.max(0, Math.round(r.balanceGross)),

    전액비과세: r.isAccumulationPhase ? Math.round(r.balance) : Math.max(0, Math.round(r.balanceInsOnly)),
  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <div className="flex-1 px-4 py-5 flex flex-col gap-4">
        ...
        {result.medicalRisk && (
          <MedicalRiskSection medicalRisk={result.medicalRisk} tier={tier} />
        )}
          ...
          <ResponsiveContainer width="100%" height={compactChart ? 320 : 280}>
            ...
            <AreaChart data={graphData} margin={compactChart ? { top: 8, right: 4, left: 0, bottom: 4 } : { top: 30, right: 8, left: 0, bottom: 0 }}>
              ...
              <ReferenceArea
                ...
                label={compactChart ? undefined : { value: '▲ 축적기', position: 'insideTopLeft', fill: '#16a34a', fontSize: 9, fontWeight: 700 }}
              ...
              {features.taxScenarioCompare && !compactChart && (
                <Area type="monotone" dataKey="전액과세"
                  ...
                  fill="url(#grossGrad)" dot={false} name="전액 과세 시나리오" connectNulls />
              )}
              ...
              {features.taxScenarioCompare && !compactChart && (
                <Area type="monotone" dataKey="전액비과세"
                  ...
                  fill="url(#insGrad)" dot={false} name="전액 비과세 시나리오" connectNulls />
              )}
              ...
              {depletionAge && inputs.monthlyInsurance > 0 && (
                <ReferenceLine x={depletionAge + 1} stroke="#fdc017" strokeDasharray="3 2" strokeWidth={0}
                  label={compactChart ? undefined : { value: '▶ 평생 끊기지 않는 종신연금 구간', position: 'insideTopRight', fill: '#eda607', fontSize: 9, fontWeight: 700 }} />
              )}
              {/* 80세 노후 간병기 리스크 구간 */}
              <ReferenceLine x={80} stroke="#dc2626" strokeDasharray="2 2" strokeWidth={1.5}
                label={compactChart ? undefined : { value: '80세 노후 간병기 리스크 구간(가정)', position: 'insideTopLeft', fill: '#dc2626', fontSize: 9 }} />
              {/* 보험 납입 종료 시점 */}
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            {!compactChart && features.taxScenarioCompare && (
              <div className="bg-emerald-50 rounded-xl p-2.5 text-center border border-emerald-100">
                ...
                <p className="text-[9px] text-emerald-500 mt-0.5">비과세 최대 활용 시</p>
              </div>
            )}
          </div>
