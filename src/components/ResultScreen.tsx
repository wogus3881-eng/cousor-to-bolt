  ...
  return (
    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <div className="flex-1 px-4 py-5 flex flex-col gap-4">
        ...
        {/* ── 경고 통계 2종: 노인 자살률 + 인플레이션 (한 카드에 통합) ── */}
        {(() => {
          ...
          return (
            <div className="rounded-2xl bg-gradient-to-br from-red-50 to-rose-50 p-4 border border-rose-200">
              ...
              <p className="text-[12px] font-bold text-rose-700 mt-1 leading-relaxed">
                ...
                경제적 어려움(27.7%)과 건강 문제(27.6%) — 두 가지 합쳐 <strong className="text-rose-900">절반 이상(55.3%)</strong>이 이 문제로 자살을 생각합니다
              </p>
            ...
            <AreaChart data={incomeExpenseData} margin={compactChart ? { top: 6, right: 4, left: 0, bottom: 4 } : { top: 10, right: 8, left: 0, bottom: 0 }}>
              ...
              {retirementBalancePension401k > 0 && (
                <Area type="monotone" dataKey="IRP연금" stroke="#6366f1" strokeWidth={2}
                  fill="url(#irpAnnuityGrad)" dot={false} name={`IRP 연금 (${pension401kAnnuityEndAge}세까지 정액 수령)`} />
              )}
              {/* 수입: 연금저축펀드 연금화 수령액 */}
              {retirementBalancePensionSavings > 0 && (
                <Area type="monotone" dataKey="연금저축연금" stroke="#a855f7" strokeWidth={2}
                  fill="url(#psAnnuityGrad)" dot={false} name={`연금저축 연금 (${pension401kAnnuityEndAge}세까지 정액 수령)`} />
              )}
            </AreaChart>
          </ResponsiveContainer>
          </div>
