      ...
      <div className="flex-1 px-4 py-5 flex flex-col gap-4">
        ...
        {(() => {
          ...
          return (
            <div className="rounded-2xl bg-gradient-to-r from-red-900 to-red-800 p-4 border border-red-700">
              ...
              <p className="text-white text-sm font-extrabold leading-snug mb-3">
                지금 {Math.floor(monthlyExpense / 10000).toLocaleString()}만원 생활비가<br />
        ...
        <div className="bg-white rounded-2xl p-5 border border-navy-100 shadow-sm">
          ...
          <div className="mt-4 flex flex-col gap-2">
            {/* 고갈 나이 핵심 정보 */}
            {depletionAge && (
              <div className="rounded-2xl bg-red-900 p-4 flex items-center justify-between">
                <div>
                  <p className="text-[11px] font-bold text-red-300 mb-1">⚠️ 현재 설계 기준</p>
                  <p className="text-white text-base font-extrabold">
                    <span className="text-red-300 text-2xl">{depletionAge}세</span>에 자산 고갈
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-red-400">은퇴 후</p>
                  <p className="text-xl font-extrabold text-amber-300">{depletionAge - retirementAge}년</p>
                  <p className="text-[10px] text-red-400">만에 소진</p>
                </div>
              </div>
            )}
            {!depletionAge && (
              <div className="rounded-2xl bg-emerald-900 p-4 flex items-center justify-between">
                <div>
                  <p className="text-[11px] font-bold text-emerald-300 mb-1">✅ 현재 설계 기준</p>
                  <p className="text-white text-base font-extrabold">
                    <span className="text-emerald-300 text-2xl">100세</span>까지 안전
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-emerald-400">자산 유지</p>
                  <p className="text-xl font-extrabold text-emerald-300">✓</p>
                </div>
              </div>
            )}
            ...
            {/* 시나리오 비교 */}
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-xl bg-navy-800 p-3 text-center">
                <div className="w-4 h-0.5 bg-red-400 mx-auto mb-1.5 rounded" />
                <p className="text-[11px] font-bold text-white">현재 설계</p>
                <p className="text-[10px] text-navy-400 mt-0.5">보험 포함</p>
                <p className="text-sm font-extrabold text-red-300 mt-1">
                  {depletionAge ? `${depletionAge}세` : '100세+'}
                </p>
              </div>
              <div className="rounded-xl bg-navy-800 p-3 text-center">
                <div className="w-4 h-0.5 bg-amber-400 mx-auto mb-1.5 rounded" style={{borderStyle:'dashed'}} />
                <p className="text-[11px] font-bold text-white">절세계좌만</p>
                <p className="text-[10px] text-navy-400 mt-0.5">보험 제외</p>
                <p className="text-sm font-extrabold text-amber-300 mt-1">
                  {noInsResult.dignityEndAge ? `${noInsResult.dignityEndAge}세` : '100세+'}
                </p>
              </div>
              <div className="rounded-xl bg-navy-800 p-3 text-center">
                <div className="w-4 h-0.5 bg-emerald-400 mx-auto mb-1.5 rounded" style={{borderStyle:'dashed'}} />
                <p className="text-[11px] font-bold text-white">은행만</p>
                <p className="text-[10px] text-navy-400 mt-0.5">예·적금</p>
                <p className="text-sm font-extrabold text-emerald-300 mt-1">
                  {bankOnlyResult.dignityEndAge ? `${bankOnlyResult.dignityEndAge}세` : '100세+'}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-xl border border-orange-200 bg-orange-50 p-2.5 flex items-center gap-2">
                <div className="w-4 h-0.5 bg-orange-400 rounded shrink-0" style={{borderStyle:'dashed'}} />
                <div>
                  <p className="text-[11px] font-bold text-orange-700">실질 가치</p>
                  <p className="text-[10px] text-orange-600">현재 구매력 기준</p>
                </div>
              </div>
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-2.5 flex items-center gap-2">
                <div className="w-4 h-0.5 bg-emerald-500 rounded shrink-0" />
                <div>
                  <p className="text-[11px] font-bold text-emerald-700">절세계획(보험 제외)</p>
                  <p className="text-[10px] text-emerald-600">ISA·IRP·증권 활용 시</p>
                </div>
              </div>
            </div>
          </div>te, useMemo, useRef, useEffect } from 'react';
        ...
        <div className="bg-white rounded-2xl p-5 border border-navy-100 shadow-sm">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <div className="bg-orange-50 rounded-xl p-2.5 text-center border border-orange-100">
              ...
              <p className="text-[10px] font-bold text-orange-700">실질 가치</p>
              <p className="text-[9px] text-orange-400 mt-0.5">현재 구매력 기준</p>
            </div>
            ...
            {!compactChart && (
              <div className="bg-gold-50 rounded-xl p-2.5 text-center border border-gold-200">
                ...
                <p className="text-[10px] font-bold text-gold-700">절세계좌만 (보험 제외)</p>
                <p className="text-[9px] text-gold-600 mt-0.5">ISA·IRP·증권만 활용 시</p>
              </div>
            )}
            {!compactChart && (
              <div className="bg-emerald-50 rounded-xl p-2.5 text-center border border-emerald-100">
                <div className="w-5 h-0.5 bg-emerald-500 mx-auto mb-1.5 rounded" style={{borderStyle:'dashed'}} />
                <p className="text-[10px] font-bold text-emerald-700">은행 예금만</p>
                <p className="text-[9px] text-emerald-500 mt-0.5">예·적금만 준비했을 때</p>
              </div>
            )}
          </div>
