...
export default function LiteLegalDisclaimer() {
  ...
  return (
    <section
      className="rounded-[20px] border border-toss-line bg-toss-canvas/80 text-[11px] leading-relaxed text-toss-sub ring-1 ring-black/[0.02]"
    ...
    >
      <button
        ...
        aria-expanded={open}
      >
        <h3 className="text-[12px] font-bold text-toss-ink">이용 안내 · 한계 고지</h3>
        <ChevronDown size={14} className={`shrink-0 text-toss-sub transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
