...
import { CheckCircle, ChevronDown, X, ShieldAlert } from 'lucide-react';
...
export default function ConsultationForm({ inputs, onSubmitSuccess }: Props) {
  ...
  const [agreedCollect, setAgreedCollect] = useState(false);
          ...
          <div className="flex flex-col gap-2 py-1">
            <div className="flex items-start gap-2">
              <input
                type="checkbox"
                id="agree-collect"
                checked={agreedCollect}
                onChange={(e) => setAgreedCollect(e.target.checked)}
                className="mt-1 h-4 w-4 rounded border-toss-line text-toss-blue accent-toss-blue"
              />
              <label htmlFor="agree-collect" className="cursor-pointer text-[11px] leading-snug text-slate-500">
            ...
            <button
              ...
              className="ml-6 flex items-center gap-1 text-[10px] font-medium text-toss-sub underline underline-offset-2"
            >
              제공 항목 자세히 보기
              <ChevronDown size={12} className={`transition-transform ${showPrivacyDetails ? 'rotate-180' : ''}`} />
            </button>
